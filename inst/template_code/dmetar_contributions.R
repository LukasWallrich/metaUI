...
se.from.p <- function(effect.size, p, N, effect.size.type = "difference", calculate.g = FALSE) {
  ...
  if (ES.type == "ratio") {
    if (calculate.g == TRUE) {
      ...
      colnames(data) <- c(
        "logEffectSize", "logStandardError", "logStandardDeviation", "logLLCI", "logULCI", "EffectSize",
        "LLCI", "ULCI"
      )
    }
  }
...
pcurve <- function(x, effect.estimation = FALSE, N, dmin = 0, dmax = 1) {
  ...
  par1 <- stringr::str_locate(raw, "\\(")[, 1]
  par2 <- stringr::str_locate(raw, "\\)")[, 1]
  comma <- stringr::str_locate(raw, ",")[, 1]
  eq <- stringr::str_locate(raw, "=")[, 1]
  ...
  main.results <- as.numeric(c(
    ktot, ksig, khalf, Zppr,
  ...
  # table_calc
  table_calc <- data.frame(
    raw, p, ppr, ppr.half, pp33, pp33.half,
    qnorm(ppr), qnorm(ppr.half), qnorm(pp33), qnorm(pp33.half)
  )
  headers1 <- c(
    "Entered statistic", "p-value", "ppr", "ppr half", "pp33%", "pp33 half",
  ...
  plotdrop <- function(var, col) {
    ...
    # Plot the dots
    plot(0:(k - 1), var,
      xlab = "", ylab = "", type = "b", yaxt = "n", xaxt = "n", main = "",
  ...
  # Input Data
  table_calc[, 1] <- NULL
  colnames(table_calc) <- c(
    "p", "ppSkewFull", "ppSkewHalf", "ppFlatFull", "ppFlatHalf", "zSkewFull", "zSkewHalf",
    "zFlatFull", "zFlatHalf"
  )
